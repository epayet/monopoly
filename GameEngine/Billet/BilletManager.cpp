...
void BilletManager::Enlever(BilletManager* billetManager)
{
    for (int i = 0; i < _billets.size(); i++)
    {
        if (_billets[i].first > 0)
            _billets[i].first -= billetManager->GetBillets()[i].first;
    }
}
...
Billet* BilletManager::GetBillet(TYPEBILLET billet)
{
    return _billets[billet].second;
}